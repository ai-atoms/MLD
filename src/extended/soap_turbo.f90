...
!**************************************************************************
! n_sites    integer      scalar           ! how many atomic sites are computed. If the entire cell has   atoms from 
                                           ! 1,2,......N n_sites in my case will be a slice 
                                           ! 1, 2, ...., i, i+1, ...., i+n_sites-1,...N
                                           ! i.e. the atoms i, i+1, ..., i+n_sites-1 : n_sites atoms 
! n_neigh    integer      vector(:)        ! are the number of neigh for each site.
!                         1:nsites         ! n_neigh(i) is  the number of neigh for i^th atomic site
! n_species  integer      scalar           ! ? number of species ? In box ? all of them ? in selection ? 
! species -
! species_multiplicity (n_sites)
!  i is from 1 to n_sites
!do k = 1, species_multiplicity(i)
!  j = species(k, i)
! n_atom_pairs - ?
! mask - ? logical mask of what ?
! rjs - ?
! thetas - ?
! phis  - ?
! alpha_max       integer   vector(:)          ! equivalent n_max for soap_classical. No bigger than 12.
!                          (1:n_species)       ! one per species
! l_max           integer   scalar             ! l from Ylm 
! rcut_hard       real      vector(:)          ! the full rcut per species 
!                          (1:n_species)       ! e.g.  real 5.0
! rcut_soft       real      vector(:)          ! soft limit of the rcut < rcut_hard
!                          (1:n_species)       ! e.g.  real 4.5
!<nf              real      vector(:)          ! I do not know what it is ... 
!                          (1:n_species)
!<global_scaling  real      vector(:)          ! I do not know what it is ... 
!                          (1:n_species)       ! it seems to scale radial componenet per species  
! atom_sigma_r    real      vector(:)          !input_soap 
!                           (1:n_species) 
! atom_sigma_r_scaling(n_species), real,       ! input_soap atom_sigma_r_scaling={{0. 0. 0.}}
! atom_sigma_t(n_species),         real,       ! input_soap atom_sigma_t = {{0.5 0.5 0.5}}
! atom_sigma_t_scaling(n_species)  real,       ! input_soap atom_sigma_t_scaling={{0. 0. 0.}}
! amplitude_scaling(n_species)     real,       ! input_soap amplitude_scaling={{1. 1. 1.}}
! radial_enhancement            integer,       ! input_soap can be 0, 1 or 2.  I suppose ?
! central_weight(n_species)        real,       ! input soap central_weight={{1. 1. 1.}}
! basis                     character(*),      ! input_soap active only for "poly3gauss"  "poly3"
!<scaling_mode  character(*) "polynomial"  or not.  param ?
! do_timing      logical                       ! time track or not
! do_derivatives logical                       ! forces or not
! compress_soap  logical                       ! compress_or_not_soap.
! compress_P_nonzero                           ! ?
! compress_P_i                                 ! ?
! compress_P_j                                 ! ?
! compress_P_el                                ! ? 
! soap                                         ! desc descriptor
!  do i = 1, n_sites
!    soap(1:n_soap, i) = soap(1:n_soap, i) / sqrt_dot_p(i)
!  end do
! soap_cart_der                                ! derivatives of desc
!-----------------------------------------------------
! compress_mode character(*)   "trivial"
! species_Z(n_species), integer
! central_index, integer 1   ex. central_index=2 Position in species_Z array
!                                occupied by the central atom species of the SOAP
!                                environment. I.e., each soap_turbo definition only
!                                sees central species of this type.
! add_species(logical)   default F   add_species=F MPORTANT: for soap_turbo there
!                        is currently no recipe to build the descriptor for all
!                        species automatically. It must be explicitly set to false.
...
! Internal variables
  ...
  logical, allocatable :: do_central(:),  skip_soap_component_flattened(:)
...
! This is to build the radial basis
  ...
  !MCM  this can be replaced by n_max = i_end(n_species)
...
!   New way to compress with a linear transformation
    n_soap = maxval(compress_P_i)
    skip_soap_component_flattened = .true.
...
! call cpu_time(time1)
!****************************
    allocate( this_soap_rad_der(1:n_soap_uncompressed) )
    allocate( this_soap_azi_der(1:n_soap_uncompressed) )
    allocate( this_soap_pol_der(1:n_soap_uncompressed) )
    k2 = 0
    do i = 1, n_sites
      do j = 1, n_neigh(i)
...
!   Internal variables
    ...
    integer :: n_sites, n_sites_supercell, i, j, k, i2,  k2, ijunk
    integer :: vindices(1:3)
