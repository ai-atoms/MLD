...
module module_LbfgsExtType_internal 
    real(kind(1.d0))     :: eps, xtol
      ...
      integer  :: MP, LP
end module module_LbfgsExtType_internal 


module LbgfsExtModule
  use MinimizerModule, only: MinimizerType
  implicit none 
  private 
  public :: LbfgsExtType 

  type, extends(MinimizerType) :: LbfgsExtType
    logical  :: precondition  
    real(kind=8) :: alpha, beta 
    integer :: m_hess 
    contains 
    procedure :: init => init_lbfgs_nocedal
    procedure :: move => move_lbfgs_nocedal
  end type LbfgsExtType

  contains

  subroutine  init_lbfgs_nocedal(this, max_iter, tol, xx)
    use module_lbfgs_nocedal, only: lbfgs_eps, lbfgs_gtol, lbfgs_m_hess, lbfgs_max_steps, &
                                  lbfgs_print 
    !ACEC! import :: LbfgsExtType
    class(LbfgsExtType), intent(inout) :: this 
    ...
    ! ---------- this normaly will be in the milady input 
    !lbfgs_m_hess = 5  
    lbfgs_eps = tol  
    !lbfgs_xtol = 1.d-16 
    lbfgs_max_steps = max_iter 
    lbfgs_print(1) = 10
    lbfgs_print(2) = 0
    lbfgs_gtol = 4.d-1
    ! ---------- this normaly will be in the milady input 

    this%m_hess = lbfgs_m_hess

    ! init lbfgs avec le bon parametres 
    ndim = size(xx,1)
    call set_up_the_lbfgs(ndim)

  end subroutine  init_lbfgs_nocedal

  subroutine move_lbfgs_nocedal (this, xx, func)
    use FunctionModule, only: FunctionType
    use   module_LbfgsExtType_internal, only:  diagco, diag_lbfgs, iprint, eps, &
                                               xtol, w_lbfgs, iflag                        
    ...
    integer :: ndim, icall 
    real(kind=8) :: functx
    real(kind=8), dimension(:), allocatable :: grad_functx    

    ndim = size(xx,1)
    if (allocated(grad_functx)) deallocate(grad_functx) ; allocate(grad_functx(ndim))
    do icall = 1, this%max_iter 
     call func%evaluate(xx, functx)
     call func%gradient(xx, grad_functx)
     call lbfgs_nocedall (ndim, this%m_hess, xx, functx, grad_functx, & 
                 diagco, diag_lbfgs, iprint, eps, xtol, w_lbfgs, iflag)
     if (iflag .le. 0) exit
    end do
    ...
    if (icall .ge. this%max_iter) then
      write (*, *) 'WARNING: the convergency is not achieved in LBFGS in call_max_lbfgs gradient claculations', this%max_iter
      if (iflag < 0) then
        write (*, *) 'LBFGS failed with iflag and info', iflag
        stop
      end if
      if (iflag == 1) then
        write (*, *) 'You should increase the maximum number of iterations call_max_lbfgs', this%max_iter
      end if
    end if
  end subroutine move_lbfgs_nocedal

  subroutine set_up_the_lbfgs(N)
    use module_LbfgsExtType_internal, only: eps, xtol, diagco, & 
                  iflag, icall, call_max_lbfgs, m_hess, & 
                  diag_lbfgs, w_lbfgs, stpmin, stpmax, &
                  MP, LP, GTOL, iprint 
    use module_lbfgs_nocedal, only: lbfgs_xtol, lbfgs_eps, &
    ...
    if (allocated(diag_lbfgs)) deallocate (diag_lbfgs) ; allocate (diag_lbfgs(n))
    ! orig to 5.
    !m_hess=100
    m_hess = lbfgs_m_hess
  ...
  subroutine lbfgs_nocedall(N, M, X, F, G, DIAGCO, DIAG, IPRINT, EPS, XTOL, W, IFLAG)
    !use module_LbfgsExtType_internal, ONLY: GTOL,STPMIN,STPMAX,MP,LP
    use module_LbfgsExtType_internal, ONLY: GTOL, LP   
...
!
  !use module_LbfgsExtType_internal, ONLY: GTOL, STPMIN,STPMAX,MP,LP
  !use module_LbfgsExtType_internal, ONLY: MP
  use module_LbfgsExtType_internal, only: MP 
...
80   FORMAT(2(I6, 1X), 3X, 3(1PD12.5, 2X))
...
60 ddot = dtemp
  ...
  !
                                                SUBROUTINE MCSRCH(N, X, F, G, S, STP, FTOL, XTOL, MAXFEV, INFO, NFEV, WA)
                                                  !use module_LbfgsExtType_internal, ONLY : GTOL,STPMIN,STPMAX,MP,LP
                                                  use module_LbfgsExtType_internal, ONLY: GTOL, STPMIN, STPMAX, LP
