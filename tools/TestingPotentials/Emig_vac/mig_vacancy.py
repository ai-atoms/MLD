...
class Migration : 
    def __init__(self, 
        ...
        for line in script_lammps.splitlines():
            if 'pair_coeff' in line:
                tokens = line.split()
                # pair_coeff * * filename species ... -> filename is after the last '*'
                idx = len(tokens) - 1
        ...
        self.path_mig = [f'{self.init_dir}/config/1vac',
                         f'{self.init_dir}/config/2vac',
                         f'{self.init_dir}/config/3vac',
                         f'{self.init_dir}/config/4vac']
    ...
    def clean_directories(self) -> None : 
        list_dir2delete = ['1vac','2vac','3vac','4vac']
        ...
        if os.path.exists(f'{self.init_dir}/mig_vac.log') :
            os.remove(f'{self.init_dir}/mig_vac.log')
    ...
    def OneMinimisation(self, path_poscar : os.PathLike[str], path_mig : os.PathLike[str]) -> Tuple[float,int] :
        self.logger(f'{self.init_dir}/mig_vac.log',
...
### MAIN
########################################
parser = argparse.ArgumentParser('MigVac')
