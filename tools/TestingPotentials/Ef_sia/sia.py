...
class Minimizer : 
    ...
    def clean_directories(self) -> None : 
        ...
        for directory in dirs_to_delete : 
            try :
                ...
                continue
        if os.path.exists(f'{self.init_dir}/sia.log') :
            os.remove(f'{self.init_dir}/sia.log')
        return 
    ...
    def OneMinimisation(self, path_poscar : os.PathLike[str]) -> Tuple[float,int] :
        self.logger(f'{self.init_dir}/sia.log',
    ...
    def GetFormationEnergy(self) -> None : 
        ...
        # Collect results: list of (name, n_sia, E_f, E_bind)
        results = []
        ...
        for dfct_poscar in self.path_poscar_dfct : 
            ...
            n_sia = nb_at_dfct - self.reference_nb_atom
            name = os.path.basename(dfct_poscar).replace('.poscar', '')
            results.append((name, n_sia, ef))
        ...
        # Reference: lowest E_f among single SIAs (n_sia == 1)
        ef_1sia_ref = min(ef for _, n_sia, ef in results if n_sia == 1)

        # Compute binding energies
        full_results = []
        for name, n_sia, ef in results:
            e_bind = n_sia * ef_1sia_ref - ef if n_sia > 1 else 0.0
            full_results.append((name, n_sia, ef, e_bind))

        # Print summary table
        header = f'{"Config":<20s} {"N_sia":>5s} {"E_f (eV)":>12s} {"E_bind (eV)":>12s}'
        sep = '-' * len(header)
        self.logger(f'{self.init_dir}/sia.log', f'\n{sep}')
        self.logger(f'{self.init_dir}/sia.log', header)
        self.logger(f'{self.init_dir}/sia.log', sep)
        for name, n_sia, ef, e_bind in full_results:
            line = f'{name:<20s} {n_sia:>5d} {ef:>12.6f} {e_bind:>12.6f}'
            self.logger(f'{self.init_dir}/sia.log', line)
        self.logger(f'{self.init_dir}/sia.log', sep)
...
### MAIN
########################################
parser = argparse.ArgumentParser('SIA')
...
local_data  = read_local_input("./sia.input")
