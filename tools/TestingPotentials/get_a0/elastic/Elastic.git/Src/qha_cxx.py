...
def gen_cxx_qha(based,cname,a0,E0,xp_vol,step_xp_vol,xp_temp,step_xp_temp):
# based - the root of the calculation
# cname
...
#Calculate elastic constants
  ...
  if (typerun == 'build'  or ( typerun == 'all') ):
    if pre_build_requested == False :
      ...
      if not os.path.exists('cname'):
         print("Probably you need to run pre_build before. Now the typerun is %s "%(str(typerun)))
         #exit(0)
  ...
  for i in range(len(xp_vol)):
    ...
    if (typerun == 'pre_build') or (typerun == 'all' ):
      ...
      if (mode == 'vasp') :
         setup_vasp(tempd,'pre'+indir,'babulk')
         os.system('pwd')
         run_vasp("out.run")
  ...
  if (typerun == 'extract') or  (typerun == 'all') or (typerun=='post_extract') :
    ...
    for it in range(len(xp_temp)):
      ...
      ft=open('int%s.dat'%str(int(xp_temp[it])),'w')
      fs=open('val%s.dat'%str(int(xp_temp[it])),'w')
      ...
      a0val,sol  = get_roots_of_the_polynom_1D(vol_min,vol_max,new_aval, xp_temp[it])
    ...
    for i in range(len(xp_temp)):
      ...
      print(("%6.1f  %14.6f   %14.6f  %14.6f  %14.6f   %14.6f  %14.6f %14.6f %14.6f %14.6f"%(xp_temp[i], axx[i], vxx[i], Bxx[i], Bs, Cvx[i]/jmol2ev, Cpx/jmol2ev, 1.e+5*alpha_vol, 1.e+5*alpha_lin, Fxx[i])))
      fab.write("%6.1f  %14.6f   %14.6f  %14.6f  %14.6f   %14.6f  %14.6f %14.6f %14.6f %14.6f \n"%(xp_temp[i], axx[i], vxx[i], Bxx[i], Bs, Cvx[i]/jmol2ev, Cpx/jmol2ev, 1.e+5*alpha_vol, 1.e+5*alpha_lin, Fxx[i]))
    fab.close()
